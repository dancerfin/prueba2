...
MIN_ATTACK_DURATION = 20  # Segundos mínimos para considerar ataque
CONSECUTIVE_DETECTIONS = 3  # Detecciones positivas requeridas

# Estructura para almacenar tiempos de detección
DETECTION_TIMES = []
ATTACK_START_TIMES = {}
...
def generate_detection_report():
    if not DETECTION_TIMES:
        print("\nNo se registraron detecciones de ataques")
        return
    
    avg_time = sum(DETECTION_TIMES) / len(DETECTION_TIMES)
    min_time = min(DETECTION_TIMES)
    max_time = max(DETECTION_TIMES)
    under_threshold = sum(1 for t in DETECTION_TIMES if t <= 180) / len(DETECTION_TIMES) * 100
    
    print("\n=== Reporte de Tiempos de Detección ===")
    print(f"Tiempo promedio de detección: {avg_time:.2f} segundos")
    print(f"Tiempo mínimo de detección: {min_time:.2f} segundos")
    print(f"Tiempo máximo de detección: {max_time:.2f} segundos")
    print(f"Porcentaje bajo 180s: {under_threshold:.2f}%")
    print(f"Total de detecciones registradas: {len(DETECTION_TIMES)}")
...
if __name__ == '__main__':
    ...
    atexit.register(generate_detection_report)
    manager.main()